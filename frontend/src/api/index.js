...
export const authAPI = {
  ...
  registerStudent: (data) =>
    api.post(
      `/auth/register/student?group_id=${data.group_id}&full_name=${encodeURIComponent(
        data.full_name
      )}${data.student_id ? '&student_id=' + data.student_id : ''}`
    ),
  registerTeacher: (data) =>
    api.post(
      `/auth/register/teacher?full_name=${encodeURIComponent(
        data.full_name
...
export const teacherAPI = {
  ...
  getLessonAttendance: (lessonId) =>
    api.get(`/teacher/lesson/${lessonId}/attendance`),
  openLesson: (lessonId) =>
    api.post(`/teacher/lesson/${lessonId}/open`),
  closeLesson: (lessonId) =>
    api.post(`/teacher/lesson/${lessonId}/close`),
  markAttendance: (lessonId, studentId, status) =>
    api.post(
      `/teacher/lesson/${lessonId}/mark/${studentId}?status=${status}`
    ),
  getSubjects: () => api.get('/teacher/subjects'),
  getGroups: () => api.get('/teacher/groups'),
  createLesson: (group_id, subject_id, room) =>
    api.post(
      `/teacher/lesson/create?group_id=${group_id}&subject_id=${subject_id}${
        room ? '&room=' + room : ''
      }`
    ),
...
export const adminAPI = {
  ...
  createGroup: (data) =>
    api.post(
      `/admin/groups/create?name=${data.name}&direction_id=${data.direction_id}&course=${data.course}`
    ),
  createSubject: (data) =>
    api.post(
      `/admin/subjects/create?name=${data.name}${
